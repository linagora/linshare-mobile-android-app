...
class SearchInteractor @Inject constructor(private val documentRepository: DocumentRepository) {
    ...
    operator fun invoke(
        ...
        return flow<State<Either<Failure, Success>>> {
            ...
            val state = Either.catch {
                ...
                .fold(
                    { Either.left(MySpaceFailure(it)) },
                    { sortDocumentListByOrderListType(it, orderListConfigurationType) }
                )
    ...
    private fun sortDocumentListByOrderListType(
        documents: List<Document>,
        orderListConfigurationType: OrderListConfigurationType
    ): Either<Failure, Success> {
        return documents.takeIf { it.isNotEmpty() }
            ?.let {
                Either.right(SearchDocumentViewState(documents.sortDocumentBy(orderListConfigurationType)))
            }
            ?: Either.left(SearchDocumentNoResult)
    }
}
