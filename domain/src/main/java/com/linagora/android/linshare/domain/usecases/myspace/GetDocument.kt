...
class GetDocument @Inject constructor(
    ...
    operator fun invoke(documentId: DocumentId) = flow<State<Either<Failure, Success>>> {
        ...
        val documentState = Either.catch { documentRepository.get(documentId) }
            .fold(
                ifLeft = { throwable -> Either.left(GetDocumentFailure(throwable)) },
                ifRight = { document -> Either.right(GetDocumentSuccess(document)) }
            )

        emitState { documentState }
    }
}
