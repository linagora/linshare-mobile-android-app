...
@Singleton
class GetSharedSpaceOrderedInteractor @Inject constructor(
    private val sharedSpaceRepository: SharedSpaceRepository
) {

    operator fun invoke(orderListConfigurationType: OrderListConfigurationType):
        ...
        return flow<State<Either<Failure, Success>>> {
            ...
            val state = Either.catch { sharedSpaceRepository.getSharedSpaces() }
                .fold(
                    { Either.left(SharedSpaceFailure(it)) },
                    { sortSharedSpaceListByOrderListType(it, orderListConfigurationType) })
