...
import com.linagora.android.linshare.domain.usecases.sharedspace.member.AddExistingMemberException
import com.linagora.android.linshare.domain.usecases.sharedspace.member.AddMemberException
...
@Singleton
class AddMemberNetworkRequestHandler @Inject constructor(
    private val retrofit: Retrofit
) : OnCatch {

    companion object {
        private val LOGGER = LoggerFactory.getLogger(AddMemberNetworkRequestHandler::class.java)
    }
    ...
    override fun invoke(throwable: Throwable) {
        ...
        when (throwable) {
            ...
            else -> throw AddMemberException(UNKNOWN_RESPONSE)
        }
    }
    ...
    private fun reactToHttpErrorResponse(httpException: HttpException) {
        ...
        errorResponse.errCode?.takeIf { it.value == AddExistingMemberErrorCode.value }
            ?.let { throw AddExistingMemberException }
            ?: throw AddMemberException(errorResponse)
    }
}
